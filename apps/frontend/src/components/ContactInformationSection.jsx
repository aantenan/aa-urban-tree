...
export function ContactInformationSection({ applicationId }) {
  const [formValues, setFormValues] = useState(CONTACT_INITIAL);
  ...
  const loadContact = useCallback(() => {
    ...
    apiJson(`/api/v1/applications/${applicationId}/contact-information`)
      .then((res) => {
        ...
        setFormValues({ ...CONTACT_INITIAL, ...(contact || {}) });
  ...
  useEffect(() => {
    if (!applicationId) {
      setFormValues(CONTACT_INITIAL);
      setSectionComplete(false);
      setContactLoading(false);
      return;
    }
    loadContact();
  }, [applicationId, loadContact]);
